...
Redux는 다음과 같은 특징을 가진 상태 관리 라이브러리입니다:

- 단일 스토어: 애플리케이션의 전체 상태를 하나의 큰 객체로 저장합니다.
- 불변성: 상태를 직접 수정하지 않고, 새로운 상태 객체를 생성하여 업데이트합니다.
- 액션: 상태 변경을 위해 액션이라는 객체를 디스패치합니다.
- 리듀서: 현재 상태와 액션을 받아 새로운 상태를 반환하는 순수 함수입니다.
- 예측 가능성: 상태 변화가 명확하고 추적 가능합니다.

장점:

- 디버깅이 쉽습니다.
- 상태 관리가 중앙화되어 있어 관리가 용이합니다.
- 시간 여행 디버깅이 가능합니다.

단점:

- 보일러플레이트 코드가 많이 필요할 수 있습니다.
- 작은 애플리케이션에는 과도할 수 있습니다.

2. XState (상태 머신 기반 접근 방식):

XState는 상태 머신과 상태 차트 개념을 사용하는 상태 관리 라이브러리입니다:

- 명시적 상태: 가능한 모든 상태를 미리 정의합니다.
- 전이: 한 상태에서 다른 상태로의 변화를 명확하게 정의합니다.
- 이벤트 기반: 상태 변화는 이벤트에 의해 트리거됩니다.
- 계층적 구조: 복잡한 상태를 중첩된 상태 머신으로 모델링할 수 있습니다.
- 병렬 상태: 독립적인 상태들을 동시에 관리할 수 있습니다.

장점:

- 복잡한 상태 로직을 시각화하고 관리하기 쉽습니다.
- 예측 가능하고 버그가 적은 상태 전이를 구현할 수 있습니다.
- 상태 관리에 대한 명확한 문서화가 가능합니다.

단점:

- 학습 곡선이 있을 수 있습니다.
- 간단한 상태 관리에는 과도할 수 있습니다.
- 상태 머신 설계에 시간이 많이 소요될 수 있습니다.

두 접근 방식 모두 장단점이 있으며, 프로젝트의 복잡성과 요구사항에 따라 적절한 방식을 선택해야 합니다.
...
5. Redux나 MobX와 같은 라이브러리가 이 방식을 사용합니다. (템플릿이나 보일러 플레이트를 필요로하는 경향이 있다)
